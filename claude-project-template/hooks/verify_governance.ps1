# verify_governance.ps1
# Self-audit: confirms the governance docs still contain their key rules (so the system can't
# silently rot away to empty headings). Also warns if a Git remote is configured.

$ErrorActionPreference = 'Stop'

$root = Split-Path -Path $PSScriptRoot -Parent
$checks = @(
    @{ File='CLAUDE.md'; Text='DOCS/ANTI_DRIFT_PROTOCOL.md' },
    @{ File='CLAUDE.md'; Text='Three-strike' },
    @{ File='CLAUDE.md'; Text='E0' },
    @{ File='DOCS\ANTI_DRIFT_PROTOCOL.md'; Text='One active scope' },
    @{ File='DOCS\ANTI_DRIFT_PROTOCOL.md'; Text='Context-reload checkpoints' },
    ...
    @{ File='DOCS\INDEX.md'; Text='Conflict order' },
